...
% g++ -w AN_STP.cpp lib/*.cpp -D%s -I include/
% 

cmd_line=sprintf('c:\MinGW\bin\gcc.exe -w AN_STP.cpp -D%s -I include/', sensor_model);

status=system(cmd_line, '-echo');   % -w no warnings

choices=[2,3,5,7];     % 1=1Hz,  2=5Hz,  3=10Hz,  4=75 isi,  5=20Hz,  6=40Hz,  7=50Hz   +astro+spine
...
fprintf('Trials=%d, ap5=%d,  ryr=%d,  astro=%d', TRIALS, ap5, ryr, astro);
...
if (ch == 6)   % 40 Hz
    disp('25 isi, 40 Hz');
    system( cmd6 , '-echo');
    AN_STP_plots(25, ex, sensor_model)
    if astro==1
        AN_STP_plots_astro(25, sensor_model)
    end
end
...
if (ch == 7)   % 50 Hz
    disp('20 isi, 50 Hz');
    system( cmd7 , '-echo');
    AN_STP_plots(20, ex, sensor_model)
    
    if astro==1
        AN_STP_plots_astro(20, sensor_model)   % isi=20 ms
    end
end
